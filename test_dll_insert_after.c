#define CLOVE_SUITE_NAME DLL_INSERT_AFTER
#include "clove.h"
...
CLOVE_TEST(FirstItemOf1ItemList)
{
	...
	dll_string_item_t* item1 = dll_insert_string_after(item0, "001");

	CLOVE_NOT_NULL(item0->node.next);
...
CLOVE_TEST(FirstItemOf2ItemList)
{
	...
	dll_string_item_t* item2 = dll_insert_string_after(item0, "002");
